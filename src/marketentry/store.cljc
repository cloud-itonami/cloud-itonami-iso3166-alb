(ns marketentry.store
  "SSoT for the Albania (ALB) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:tax-arrears-amount` is ALL (Albanian Lek) -- ground truth for the
  `tax-arrears-exceeds-de-minimis` flagship check (Ligji Nr. 162/2020
  Neni 76(2)(c)), compared against a FLAT statutory constant, unlike
  Bulgaria's turnover-scaled formula -- so no `:annual-turnover` field
  is carried here at all (it would be honestly unused)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication Sh.p.k." :portal "app.gov.al"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tax-arrears-amount 0
             :requires-nuis? true :nuis-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ALB" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "app.gov.al"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tax-arrears-amount 0
             :requires-nuis? true :nuis-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "app.gov.al"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ALB" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export Sh.p.k." :portal "app.gov.al"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tax-arrears-amount 15000
             :requires-nuis? true :nuis-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ALB" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "app.gov.al"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tax-arrears-amount 0
             :requires-nuis? true :nuis-verified? false
             :drafted? false :submitted? false
             :jurisdiction "ALB" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "app.gov.al"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-nuis?)                (assoc :engagement/requires-nuis? requires-nuis?)
    (some? nuis-verified?)                (assoc :engagement/nuis-verified? nuis-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/tax-arrears-amount
   :engagement/requires-nuis? :engagement/nuis-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-nuis? (boolean (:engagement/requires-nuis? m))
     :nuis-verified? (boolean (:engagement/nuis-verified? m))
