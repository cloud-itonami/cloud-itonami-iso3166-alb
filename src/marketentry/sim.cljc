...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (ALB, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up tax-arrears-exceeds-de-minimis) ==")
    ...
    (println "== filing/submit eng-4 (tax-arrears-exceeds-de-minimis -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up nuis-unverified) ==")
    ...
    (println "== filing/submit eng-5 (nuis-unverified -> HARD hold) ==")
