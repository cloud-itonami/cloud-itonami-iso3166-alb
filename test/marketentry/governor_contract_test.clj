  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication Sh.p.k."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication Sh.p.k." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest tax-arrears-exceeds-de-minimis-is-held-and-unoverridable
  (testing "tax/social-security arrears above the Neni 76(2)(c) flat de-minimis threshold -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:tax-arrears-exceeds-de-minimis} (-> (store/ledger db) last :basis)))
...
(deftest nuis-unverified-is-held-and-unoverridable
  (testing "unverified NUIS (Numri Unik i Identifikimit të Subjektit) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:nuis-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
